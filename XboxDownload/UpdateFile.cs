    ...
    internal class UpdateFile
    {
        ...
        public static async void Start(bool autoupdate, Form1 parentForm)
        {
            ...
            var tasks = proxies2.Select(proxy =>
                Task.Run(() =>
                {
                    string url = $"{proxy}{project}/releases/download/v2/XboxDownload.dll.md5";
                    string html = ClassWeb.HttpResponseContent(url, "GET", null, null, null, 30000, "XboxDownload");
                    if (html.Length == 32)
                    {
                        md5 = html;
                        cts.Cancel();
            ...
            {
                Start2(autoupdate, parentForm);
                return;
            }

            if (!string.Equals(md5, GetPathHash(Application.ExecutablePath)))
            {
                bool isUpdate = false;
            ...
            string? fastestUrl = await ClassWeb.GetFastestProxy(proxies1.Concat(proxies2).ToArray(), $"{project}/releases/download/v2/XboxDownload.zip", new() { { "Range", "bytes=0-10239" } }, 3000);
            if (fastestUrl != null)
            {
        ...
        public static async void Start2(bool autoupdate, Form1 parentForm)
        {
            Properties.Settings.Default.NextUpdate = DateTime.Now.AddDays(7).Ticks;
            Properties.Settings.Default.Save();
            ...
            Version currentVersion = new(Assembly.GetEntryAssembly()?.GetCustomAttribute<AssemblyFileVersionAttribute>()?.Version!);
            if (latestVersion > currentVersion)
            {
                bool isUpdate = false;
                parentForm.Invoke(new Action(() =>
                {
                    isUpdate = MessageBox.Show($"已检测到新版本 {latestVersion}，是否立即更新？", "软件更新", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) == DialogResult.Yes;
                    if (!isUpdate) parentForm.tsmUpdate.Enabled = true;
                }));
                if (!isUpdate) return;
            }
            else
            {
                parentForm.Invoke(new Action(() =>
                {
                    if (!autoupdate) MessageBox.Show($"软件已经是最新版本 {currentVersion}。", "软件更新", MessageBoxButtons.OK, MessageBoxIcon.None);
                    parentForm.tsmUpdate.Enabled = true;
                }));
                return;
            }
