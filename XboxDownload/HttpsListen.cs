...
namespace XboxDownload
{
    class HttpsListen
    {
        ...
        public class SniProxy
        {
            ...
            public IPAddress[]? IPv4s { get; set; }
            public IPAddress[]? IPv6s { get; set; }
                            ...
                            if (jeHosts.ValueKind == JsonValueKind.Array)
                            {
                                ...
                                foreach (var str in jeHosts.EnumerateArray())
                                {
                                    ...
                                    SniProxy proyx = new()
                                    {
                                        Branch = branch,
                                        Sni = sni,
                                        IPv4s = lsIPv4.Count >= 1 ? lsIPv4.ToArray() : null,
                                        IPv6s = lsIPv6.Count >= 1 ? lsIPv6.ToArray() : null,
                                        CustomIP = customIp
                                    };
                                    if (host.StartsWith('*'))
                                    {
                                ...
                                switch (_host)
                                {
                                    ...
                                    default:
                                        if (Properties.Settings.Default.SniProxy)
                                        {
                                            ...
                                            {
                                                string? fastestUrl = await ClassWeb.GetFastestProxy(UpdateFile.proxies1, _url, new() { { "Range", "bytes=0-10239" } }, 3000);
                                                if (fastestUrl != null)
                                                {
                                                    bFileFound = true;
                                                    StringBuilder sb = new();
